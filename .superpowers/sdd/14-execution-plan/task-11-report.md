...
### 1. 보정 분기의 "타깃-사유 불일치" 차단 (`cardgen.py`)

LLM의 `ai_rank_target`이 미매칭이거나 추진중/완료(BLOCKED) 후보라 `_first_available()`로 타깃을
바꾸는 보정 분기에서, comparison·reasons·expected_effect는 LLM 원문이 남아 "타깃=A, 사유=B"
카드가 저장될 수 있었다(감사 가능성 최악 케이스). 보정 시 `out = _fallback_ai(cands, cards)`로
**텍스트까지 통째 교체**하도록 수정 — fallback의 사유·비교문은 교체된 타깃 기준으로 생성된다.

### 2. 가정 문구 판정 강화 (`cardgen.py _ensure_assumption`)

종전 판정("가정" 키워드 포함)으로는 고정 문구 없는 카드가 통과 가능 →
`ASSUMPTION_NOTE("가정 기반 전망이며 실제와 다를 수 있음") in text` 전체 문구 판정으로 강화,
없으면 append. (트레이드오프: LLM이 "가정 기반 전망, 실제와 다를 수 있음" 같은 변형 문구를 쓰면
고정 문구가 한 번 더 붙지만, 절대 규칙 3의 고정 문구 보장이 우선.)

### 3. 카드 B 조정 근거 재배치 (`seed_demo.py` + 11 §1 2행)

추진중 카드 A가 Score 5위 카페라 "중복 회피"만으로는 1위 숙박업이 아닌 2위 소매점을 고른 이유가
설명되지 않는 지적(중복 회피는 숙박업에도 성립). reasons·comparison·대본 2행의 무게를
① **성수기(계절성 신호) 내 착수 확실성** — 숙박업은 가맹 협상·시설 확인 소요가 길 가능성(추측 표기)
② 추진중인 카페 확충과의 **동선 시너지**(+업종 비중복) ③ 업종공백도 1.0 순으로 재배치.
실데이터 수치(0.67·0.65·격차 0.02)는 그대로.

### 검증 (함수 단위 — Docker 불필요, 리뷰 지시)

- [1] `llm.generate_json`을 mock으로 금지 타깃("영월군 카페"=추진중) + "LLM원문" 표식 텍스트를
  반환하게 강제 → 저장 카드: 타깃 영월군 숙박업(비차단 최상위)으로 보정, ai 텍스트에 "LLM원문"
  0건·"규칙 기반" 포함·comparison이 실제 타깃(숙박업) 지칭 — 통째 교체 확인
- [2] 고정 문구 없이 "가정" 키워드만 있는 mock 응답 → 고정 문구 append 확인, 고정 문구가 이미
  있으면 무변경 확인 (위 [1] 케이스에서 카드 저장 경로 결합 동작도 확인)
- [3] 문구 재확인 + `py_compile` 통과
