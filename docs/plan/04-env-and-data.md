...
**한 계정의 인증키 하나로 4개 API를 모두 호출한다. 단, API별로 활용신청 승인이 각각 필요하다.**
...
### 1-2. 활용신청 4건 (각각 검색 → 상세 페이지 → [활용신청] 버튼)
...
| ④ | `기상청 단기예보` | 기상청_단기예보 조회서비스 (초단기실황 `getUltraSrtNcst`) | 방문객 위젯 "오늘의 추천"의 현재 기온·강수 |

- ④만 파이프라인이 아니라 **FE 서버**(Vercel·Docker frontend 컨테이너)가 호출한다 —
  같은 `DATA_GO_KR_API_KEY`를 **서버 전용 환경변수**로 넣는다(`NEXT_PUBLIC_` 접두사 금지, 09 §2).
...
지도 타일은 지오코딩과 무관한 별개 키다 — 화면 지도는 Kakao Maps JS를 쓰므로 `NEXT_PUBLIC_KAKAO_MAP_KEY`(JavaScript 키)와 Web 도메인 등록이 필요하다(02 문서).
