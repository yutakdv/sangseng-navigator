...
> ⚠ **이 문서는 2026-08-04 시점의 검토 기록이다.** §3-1·§3-2 의 SAM/Lambda 관련 지적은 2026-08-11 ECS Fargate 이전으로 **대상 자체가 사라졌다**(`infra/template.yaml`·`deploy-backend.sh` 삭제). 기록으로 남기므로 내용을 고치지 않는다 — 현행 배포 정본은 [09](09-deployment.md)다.
