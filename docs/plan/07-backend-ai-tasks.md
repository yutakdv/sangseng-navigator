...
| ⑦ 타깃 요일 패턴 | `usage_daily.json` (2026-08-08 신설, 참고용·컷 가능). 타깃 (읍×표시업종)의 요일별 하루 평균·최대 요일·주중 대비 주말 배율. **확충 후보는 공백 업종이라 타깃 자체 실적이 0인 경우가 기본**이므로, 그때는 읍 전 업종 패턴으로 폴백하고 그 사실을 집계 대상 라벨에 명시한다. 프롬프트에서 **순위·대상 조정 근거로 쓰는 것을 금지**하고 방문 수요가 몰리는 시점 관련 리스크 서술에만 쓰게 한다 |

> 프롬프트(`prompts.py` A-1)의 번호는 위 표와 1:1이 아니다 — 프롬프트는 "서버가 확정한 제안 대상과
> 선택 규칙"을 별도 입력 2로 세어 총 8개를 나열한다. 표는 **데이터 출처** 기준, 프롬프트는 **모델이 읽는
> 순서** 기준이라 그렇다. 코드(`cardgen._build_inputs`)의 키 접두 번호가 프롬프트 번호와 같다.
...
> **미사용 (2026-08-08)** — `backend/app/prompts.py`에서 제거했다. 방문객 위젯의 추천 문구는
> LLM이 아니라 `routes/widget.py`의 결정론 문구를 쓴다(실명 점포의 맛·분위기·메뉴를 추정하지
> 않기 위한 설계 결정, 05 §4). 아래 원문은 설계 기록으로만 남긴다.
