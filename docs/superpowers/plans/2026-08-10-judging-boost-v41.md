...
| 6 | 원화 임팩트 (공시 금액 출처 확인 후) — **2026-08-10 조사·사용자 확정으로 갱신됨(Task A5)** | 원 계획이 인용하려던 언론 보도 발생액·비율은 강원랜드 공식 문서와 대조한 결과 불일치가 확인돼 폐기됐다(조사 근거는 `d1-sources-research.md`). 강원랜드 자신이 공표한 2024년 수치는 지역 사용금액 355억 원·지역사용률 29.4%(강원랜드 2024년도 지속가능경영보고서)이고, 발생액 총액은 어느 공식 문서에도 없어 옛 금액 환산 계산의 근거가 소멸함. **건수 지표와 금액 지표를 나란히 놓지 않는다는 규칙**(05:69, CLAUDE.md 절대 규칙 2)은 그대로 유지 | **화면(UI)은 건수 기준만**: 1%p ≈ 연간 +24,787건(연인원 2,478,656 기준, Task A2). **옛 금액 환산 계산은 폐기.** README 참고 문단·docs/plan/23에는 대신 "2025년 하이원포인트 지역 사용금액 415억 원(전년 대비 +16.8%, 역대 최대 — 강원랜드 2025년도 지속가능경영보고서)"이라는 공식 최신 실적 추세를 인용한다(Task D1·D4). 출처는 이미 확정됨(`.superpowers/sdd/2026-08-10-judging-boost-v41/d1-sources-research.md`) |
...
- **절대 규칙(CLAUDE.md)**: ① UI에 Gini·HHI 노출 금지("지역 소비 집중도"/"업종별 소비 분산도") ② "지역 전환율"에는 항상 `근사 지표` 배지 + 29.4% 금액 지표(강원랜드 2024년도 지속가능경영보고서 공표치, Task A5 갱신)와 구분 ③ 모든 시뮬레이션 출력에 "가정 기반 전망이며 실제와 다를 수 있음" ④ AI는 제안만("의사결정 근거 제공") ⑤ 원래 Score 순위 항상 병기 ⑥ 처방은 하이원포인트 가맹점 확충으로 고정(국세청 데이터는 진단 참고용)
...
**옛 금액 환산 계산은 폐기한다** — 근거였던 콤프 발생액 총액이 언론 보도에만 있고 강원랜드 공식 문서에는 없음이 확인됐다(2026-08-10 조사·사용자 확정, Task A5). Task D4의 docs/plan/23에는 대신 "2025년 하이원포인트 지역 사용금액 415억 원(전년 대비 +16.8%, 역대 최대 — 강원랜드 2025년도 지속가능경영보고서)"이라는 공식 최신 실적 추세를 참고 기록으로 담는다 — **화면에는 넣지 않는다**(건수 지표와 금액 지표를 나란히 놓지 않는 규칙, 05:61-69 준수). 출처는 이미 확정됨(`.superpowers/sdd/2026-08-10-judging-boost-v41/d1-sources-research.md`).
...
## 왜 필요한가
[3문장 — 첫 문장에 석탄산업전환지역 개발 지원에 관한 특별법(구 폐광지역개발지원특별법)·내국인 카지노 독점의 반대급부 명시.
둘째 문장에 임팩트 한 줄: "지역 전환율을 1%p 끌어올리면 연간 지역 사용이 약 +2.5만 건(건수 기준·추정)".
셋째 문장에 투트랙(공급=가맹 확충 / 수요=페이백) 해결 요약]

**법령 인용·공시 수치는 2026-08-10 조사·사용자 확정으로 이미 확정됐다(Task A5, 아래 그대로 사용).**

> **석탄산업전환지역 개발 지원에 관한 특별법**(법률 제21254호, 2025.12.30. 공포·2026.3.31. 시행;
> 구 「폐광지역 개발 지원에 관한 특별법」) **제1조(목적)** — "이 법은 석탄산업의 사양화로 인하여 낙후된
> 석탄산업전환지역의 경제를 진흥시켜 지역 간의 균형 있는 발전과 주민의 생활 향상을 도모함을 목적으로 한다."
> (법령ID 고정 링크 — 개정과 무관하게 항상 현행판으로 연결: https://www.law.go.kr/LSW/lsInfoP.do?urlMode=lsInfoP&lsId=000288)

> **강원랜드 지역상생 공시** — 2024년 하이원포인트 지역 사용금액 355억 원, 지역사용률 29.4%
> (강원랜드 2024년도 지속가능경영보고서). 2025년에는 415억 원(전년 대비 +16.8%, 역대 최대)으로
> 늘었다(강원랜드 2025년도 지속가능경영보고서, 2026-07 발간) — 이 보고서는 발생액 총액·비율(%)은
> 공개하지 않는다.

**참고 — 금액 기준 추세로 보면**: 하이원포인트 지역 사용금액은 2023년 328억 원(28.1%) → 2024년 355억 원
(29.4%) → 2025년 415억 원(+16.8%, 역대 최대)으로 꾸준히 늘고 있다. 강원랜드가 발생액 총액을 공개하지
않아 "1%p당 금액" 환산은 계산하지 않고, 이 증가 추세 자체를 인용한다. 본 서비스의 "지역 전환율"은
건수 기준 근사 지표로 위 금액 지표와는 종류가 다른 별개 지표다(비교 불가 고지 — 05:61-69 준수).
이 문단은 README·발표 전용이며 **앱 화면에는 넣지 않는다.**
...
**주의**: README에서 "지역 전환율" 언급 시 근사 지표임을 병기하고, 29.4%(금액 기준, 강원랜드 2024년도 지속가능경영보고서)와 나란히 비교하지 않는다(절대 규칙 2 — 현 README에는 이 규칙이 미적용 상태였음). Gini·HHI 용어 금지. 지도 스택 서술 정합화: "카드 상세 MapLibre GL + OpenFreeMap / 방문객 위젯 Kakao Maps JS(키 없으면 정적 폴백)".

- [ ] **Step 3: 외부 인용 출처 — 이미 확정됨, 아래 값을 그대로 쓸 것.** 실행 세션이 WebSearch·WebFetch로 ① 특별법 제1조 원문(국가법령정보센터 law.go.kr) ② 강원랜드 콤프/하이원포인트 지역 사용 수치(2023~2025)를 조사해 사용자 확정을 받았다(2026-08-10, 조사 근거는
  `.superpowers/sdd/2026-08-10-judging-boost-v41/d1-sources-research.md`, 반영은 Task A5). 확정 출처: **법령** — 「석탄산업전환지역 개발 지원에 관한 특별법」(법률 제21254호, 2026.3.31. 시행; 구 「폐광지역 개발 지원에 관한 특별법」), 법령ID 고정 링크
  `https://www.law.go.kr/LSW/lsInfoP.do?urlMode=lsInfoP&lsId=000288`. **공시 수치** — 강원랜드 2024년도
  지속가능경영보고서(2024년 355억 원/29.4%), 강원랜드 2025년도 지속가능경영보고서(2025년 415억 원,
  +16.8%, 2026-07 발간). 이 값들을 인용 블록·참고 문단·docs/plan/23에 그대로 반영한다("출처 확인 중" 표기 불필요)
...
- [ ] **Step 1: 23 문서 작성** — 구성(각 절 간결하게):
  ...
  3. 임팩트 계산 가정: per_pp = 연인원 2,478,656 × 1% = 24,787건. **금액 기준 참고 추세**(2023년 328억 원/28.1% →
     2024년 355억 원/29.4% → 2025년 415억 원/+16.8%·역대 최대 — 강원랜드 지속가능경영보고서. 발생액 총액
     비공개로 "1%p당 금액" 환산은 계산하지 않음. 지표 종류가 달라 화면 미노출, 05:61-69 근거, README 참고
     문단과 동일 수치 + 이미 확정된 공시 출처 링크, `.superpowers/sdd/2026-08-10-judging-boost-v41/d1-sources-research.md`)
...
1. ~~공시 출처 링크 확정 승인~~ — 해소됨. 2026-08-10 조사·사용자 확정 완료(Task A5). 법령 링크·공시 수치 모두
   확정되어 D1 Step 3에 반영 — 더 이상 블로킹 항목 아님
