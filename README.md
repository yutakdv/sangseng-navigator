...
| ④ **담당자 승인** | AI는 제안만, 확정은 사람이 — 승인·보류·반려가 모두 기록되는 Action Card |
| ⑤ **반전 장면** | 페이백 민감도를 올렸는데 처방이 **수요 측 → 공급 측으로 뒤집히는** 10초 |
| ⑥ **방문객 반영** | 승인 결과가 방문객 위젯의 추천 순서에 그대로 나타난다 |
...
| 8 | **전체 지역 현황** `/dashboard` | 6개 지역 전체 기준 진단 — 현황 / 추이·분포 / 제안 근거 세 뷰를 탭으로 전환(`?view=`) |
...
담당자의 결정이 방문객 화면의 추천 순서로 이어집니다. 로그인 없이, 요일 패턴과 현재 날씨까지 반영합니다.
...
| **AI 정책 나침반** | 읍·시 1단계 → 반경 500m 2단계 스코어링 + AI가 추진현황·계절성·형평성을 종합해 확충 우선순위 제안. **AI는 제안만, 확정은 담당자 승인** (원 Score 순위 상시 병기) |
...
LLM은 **문장만** 씁니다. 대상 선정·점수·순위·상태는 서버가 정하고, AI 응답에 섞여 온 숫자는 정본 데이터로 다시 채워 넣습니다. LLM 호출이 실패하면 룰 기반 문구로 폴백해 화면이 비지 않습니다.
...
| **완성도** | 8개 화면 · 로딩/404/에러 경계 · mock ↔ 실 API 이중 모드 · 모바일 대응 | 전 화면 |
