...
실행 후 생성). 진단·스코어링·추천은 **배포 환경에서 외부 API 호출 없이** 이 정적 데이터로 동작합니다
(경진대회 권장 "정적 데이터 방식"). 실시간 호출은 방문객 위젯 "오늘의 추천"이 쓰는
**기상청 초단기실황 한 건뿐**이며, 호출이 실패하면 날씨 줄만 빠지고 요일 패턴 추천은 그대로 뜹니다.
