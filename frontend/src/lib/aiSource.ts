 ...
 *
 * AI 3종은 라벨을 **"AI 서술 · <검증 범위>"** 한 어형으로 맞춘다. 앞은 "누가 문장을 썼는가",
 * 뒤는 "그중 무엇이 서버 재검증을 거쳤는가"다. 이전 라벨("AI 생성 · 서버 검증됨")은 뒤 절이
 * 범위를 말하지 않아 서술 문장까지 검증된 것처럼 읽혔는데, 실제 재검증 대상은 수치·순위뿐이고
 * 서술 문장은 아니다 — 라벨이 설명보다 세게 말하면 절대 규칙 4의 신뢰가 거꾸로 깎인다.
 */
export const NARRATIVE_SOURCE_TEXT: Record<NarrativeSourceKind, { label: string; note: string }> = {
  ai: {
    label: "AI 서술 · 수치·순위 서버 검증",
    note: "후보명·Score·순위·추진 상태·도로 시간은 서버가 정본 데이터로 다시 만들었습니다. 비정량 리스크·유의사항은 AI가 쓴 서술 그대로이며 재검증 대상이 아닙니다.",
  },
  ai_partial: {
    label: "AI 서술 · 수치만 서버 고정",
    note: "3·5·7% 시나리오와 개선폭은 서버가 고정한 값이고, 비교 문장과 근거 문구는 AI가 쓴 원문 그대로입니다. 재검증할 후보명·순위·도로 시간이 없는 카드라 확충 카드의 '수치·순위 서버 검증'과 구분합니다.",
  },
  ai_unverified: {
    label: "AI 서술 · 서버 검증 없음",
    note: "반대 관점 3항은 AI가 작성한 서술이며, 후보명·Score·순위처럼 서버가 정본 데이터로 다시 검증하는 필드가 아닙니다. 내용은 참고용으로 읽고 승인 여부는 담당자가 판단합니다.",
  },
  rule_fallback: {
