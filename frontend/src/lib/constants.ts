...
export const PROXY_NOTE =
  "분자=지역 사용 건수, 분모=입장 연인원(교대 합산)으로 단위가 달라 비율이 아닌 근사 지표입니다. 강원랜드가 공개한 금액 기준 지역 사용 비율(2024년 29.4%)과는 다른 지표입니다.";

/**
 * 소표본 보호 고지 (v4.1 Phase 5) — 파이프라인 P10이 발행하는 privacy_meta.note와
 * **글자 단위로 같은 문구**다. 바꿔야 하면 pipeline/p10_privacy.py NOTE를 먼저 고치고 여기를 맞춘다.
 * privacy_meta가 없는 구형 응답에서도 배지가 사유를 말할 수 있도록 기본 툴팁으로 쓴다.
 */
export const PRIVACY_NOTE =
  "가맹점 5곳 미만 셀의 건수는 비공개. 합계는 100 단위 반올림으로 차분 복원 정밀도를 낮춤(완전 차단은 아님). 비율·순위·스코어는 반올림 전 원값으로 계산됨.";
