...
/** 이 값 이상(=중립 가정)에서 부하 상위 셀의 처방을 공급 측으로 돌린다 */
const FLIP_BETA = 0.3;
    ...
    <div ref={root} className="flex flex-col gap-4">
      ...
      >
        ...
        {flipped ? (
          <p className="u-body mt-3 text-admin-text-soft">
            같은 재원을 써도 이 셀이 가져가는 몫은 포화되지 않은 셀의 절반입니다. 병목이 수요가 아니라
            가맹점 수라는 뜻이므로, 이 구간에서는 하이원포인트 가맹점 확충을 먼저 놓는 편이 근거가
            섭니다.
          </p>
        ) : null}
      ...
      <div className="rounded-xl border border-admin-border p-4">
        ...
        {cell.tier === "high" ? (
          <>
            ...
            <p className="u-body mt-2 text-admin-text-soft">
              ...
              커집니다 — 다만 늘어나는 속도가 절반이라, 같은 재원으로 얻는 몫의 격차가 민감도 가정을
              올릴수록 벌어집니다(현재 {gapPp.toFixed(2)}%p).
            </p>
          </>
        ) : (
          <p className="u-body mt-2 text-admin-text-soft">
        ...
        <p className="u-note mt-2 border-t border-admin-border pt-2">
          판정 규칙 — 부하 상위 구간 셀에서 민감도 가정이 중립(0.30) 이상이면 처방을 공급 측으로
          돌립니다. 효과를 낙관적으로 볼수록 포화에 흡수되는 몫이 커지기 때문입니다. 보수(0.15)에서는
          흡수되는 몫이 작아 수요 측 우선을 유지합니다.
        </p>
      </div>
