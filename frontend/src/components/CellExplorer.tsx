      ...
      >
        ...
        {flipped ? (
          <p className="u-body mt-3 text-admin-text-soft">
            {/* 이 문장은 실측 결론이 아니라 팀이 정한 감쇠·기준선에서 나오는 판정이다 —
                단정하면 화면이 스스로 댄 근거(가정)보다 센 말을 하게 된다 */}
            같은 재원을 써도 이 셀이 가져가는 몫은 포화되지 않은 셀의 절반이라고 가정합니다. 이
            조건에서 포기하는 몫은 {gapPp.toFixed(2)}%p로 전환 기준선 {FLIP_GAP_PP.toFixed(2)}%p를
            넘습니다 — 이 가정에서는 수요보다 가맹점 수가 먼저 걸린다고 읽히므로, 이 구간에서는
            하이원포인트 가맹점 확충을 먼저 검토할 근거가 됩니다. 감쇠·기준선은 팀이 설정한 값이라
            실측으로 확인하기 전에는 결론이 아니라 가정 탐색입니다.
          </p>
        ) : null}
