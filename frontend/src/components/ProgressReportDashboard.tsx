...
import type { CardProgress, ProgressMetricChange, ProgressReport } from "@/types";
 ...
 */
/**
 * 유형 하나의 단계 분포 — **순서가 있는 트랙**으로 그린다.
 *
 * 예전에는 단계마다 알약(테두리 + 그 안에 ProgressChip + 건수)을 만들어 줄바꿈으로 흘렸는데
 * 두 가지가 틀렸다: ① 알약 안에 알약이 들어가 테두리가 두 겹이고, ② 0건 단계가 1건 단계와
 * 똑같은 무게로 그려져 여섯 칸 중 값이 있는 두 칸이 묻혔다. 게다가 줄바꿈이 일어나면
 * "후보 접촉 → … → 완료"라는 순서 자체가 화면에서 끊긴다 — 순서가 이 데이터의 뜻인데도.
 *
 * 그래서 단계를 같은 폭의 칸으로 한 줄에 세우고, 칸 사이를 하나의 가는 선으로 이어 흐름을
 * 형태로 보여 준다. 건수가 주인공이라 큰 숫자를 위에, 단계 이름을 아래 보조로 둔다.
 * **0건은 물러난다** — 흐릿한 색과 빈 진행선으로 두고, 값이 있는 칸만 라벤더(완료는 초록)로
 * 채운다. 색은 상태를 인코딩할 때만 쓴다는 원칙 그대로다 (13 §4).
 *
 * `보류`는 흐름 밖(이탈)이라 세로 구분선 뒤로 떼어 놓는다 — flowLabel이 "(+보류)"로
 * 괄호에 넣어 말하던 것을 배치로 옮겼다. 그래서 flowLabel 문구는 여기서 더 쓰지 않는다
 * (같은 단어를 트랙 바로 위에 한 번 더 적는 중복이었다).
 */
function DistributionLayer({ layer }: { layer: WorkflowLayer }) {
  const path = layer.stages.filter((stage) => stage !== "보류");
  const hasHold = layer.stages.includes("보류");
    ...
    <div className="rounded-xl bg-admin-surface-sunken px-3 py-3">
      ...
      <div className="mt-2.5 flex items-stretch gap-2">
        {/* 진행 순서 — ol이라 스크린리더에도 "몇 번째 단계"가 그대로 전달된다.
            ...
            대신 칸을 줄이고 단계 이름을 두 줄로 접는다 — 여섯 칸이 한눈에 다 보이는 쪽이 낫다. */}
        <ol className="flex min-w-0 flex-1 gap-0">
          {path.map((stage, i) => (
            <StageCell
              key={stage}
              stage={stage}
              count={layer.counts[stage] ?? 0}
              first={i === 0}
        ...
        {hasHold ? (
          <>
            ...
            <StageCell stage="보류" count={holdCount} standalone />
          </>
        ) : null}
      </div>
    </div>
